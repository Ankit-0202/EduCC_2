...
struct Macro {
  ...
  std::vector<std::string> parameters; // List of parameter names (for variadic
                                       // macros, the last is "__VA_ARGS__")
  std::string
      replacement; // The replacement text (as written in the macro definition)
};
...
private:
  ...
  // 'disabled' holds macro names that are currently disabled (to prevent
  // recursive re-expansion).
  std::string
  expandTokens(const std::string &source,
               const std::unordered_map<std::string, bool> &disabled);

  // Expand a function-like macro using its arguments.
  std::string
  expandFunctionMacro(const Macro &macro,
