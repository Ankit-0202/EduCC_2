...
private:
    ...
    // Process a file: handle includes, then pass through conditional and macro expansion.
    std::string processFile(const std::string &path);

    // Process include directives.
    std::string processIncludes(const std::string &source, const std::string &currentFile);
