...
llvm::Value *CodeGenerator::generateLValue(const ExpressionPtr &expr) {
  ...
  // If it's a MemberAccess, assume the base is a union and the member is at
  // offset 0.
...
// Global declarations are processed first (including enum and union
// declarations), then function declarations.
//
    ...
    if (auto varDecl = std::dynamic_pointer_cast<VariableDeclaration>(decl)) {
      ...
      if (varDecl->initializer) {
        if (auto lit = std::dynamic_pointer_cast<Literal>(
          ...
          // If the initializer type doesn't match the variable type, insert a
          // conversion.
...
llvm::Type *CodeGenerator::getLLVMType(const string &type) {
  ...
  // Support enum types (treated as int)
    ...
    return Type::getInt32Ty(context);
  // Support union types: look up the union definition by tag and determine
  // maximum member size.
...
void CodeGenerator::generateFunction(
  ...
  if (function->empty()) {
    ...
    // The function was already defined => we should have thrown above, but just
    // in case:
    throw runtime_error(
...
// We create an alloca and store the initializer—if the initializer’s type does
// not match, we insert a conversion (e.g. from double to float).
//
